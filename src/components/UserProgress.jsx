...
export function UserProgress() {
    ...
    if (isLoading) {
        return (<div className="p-4 bg-card rounded-2xl border border-border card-elevated">
    ...
    return (<div className="space-y-4">
      ...
      <div className="p-4 bg-card rounded-2xl border border-border card-elevated">
        <div className="grid grid-cols-2 gap-4">
          {/* Streak */}
          <button type="button" onClick={() => setStreakOpen(true)} className="flex items-center gap-3 p-3 bg-accent/10 rounded-xl text-left hover:bg-accent/15 transition-colors" aria-label="Open streak calendar">
        ...
        {/* Heart Timer */}
        {hearts < 5 && (<div className="mt-4">
            <HeartTimer currentHearts={hearts} regenStartedAt={profile?.heart_regeneration_started_at || null}/>
          </div>)}
      </div>
