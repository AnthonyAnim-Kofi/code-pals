...
export function AdminEditDialog({ open, onOpenChange, title, fields, initialValues, onSave, loading }) {
      ...
      <DialogContent className="bg-slate-800 border-slate-700 text-white max-h-[85vh] flex flex-col">
        ...
        <div className="space-y-4 py-4 overflow-y-auto flex-1 min-h-0 pr-2">
          {fields.map((field) => (<div key={field.key}>
              ...
              {field.type === "textarea" ? (<Textarea value={values[field.key] || ""} onChange={(e) => setValues({ ...values, [field.key]: e.target.value })} className="bg-slate-700 border-slate-600 mt-1"/>) : field.type === "select" ? (<select value={values[field.key] || ""} onChange={(e) => setValues({ ...values, [field.key]: e.target.value })} className="w-full mt-1 px-3 py-2 rounded-md bg-slate-700 border border-slate-600 text-white">
                  {field.options?.map((opt) => (<option key={opt.value} value={opt.value}>{opt.label}</option>))}
                </select>) : (<Input value={values[field.key] || ""} onChange={(e) => setValues({ ...values, [field.key]: e.target.value })} className="bg-slate-700 border-slate-600 mt-1"/>)}
            </div>))}
        </div>
        <DialogFooter>
