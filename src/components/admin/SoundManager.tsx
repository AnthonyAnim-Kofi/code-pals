...
export function SoundManager() {
  ...
  const handlePlay = (url: string, key: string) => {
    if (playing === key) {
  ...
  const handleUrlSave = async (sound: any) => {
  ...
  const handleFileUpload = async (sound: any, file: File) => {
      ...
      <div className="space-y-4">
        {sounds.map((sound) => (
          <SoundRow
            key={sound.id}
            sound={sound}
            urlInput={urlInputs[sound.id] ?? sound.sound_url ?? ""}
            onUrlChange={(val) => setUrlInputs((prev) => ({ ...prev, [sound.id]: val }))}
            onSave={() => handleUrlSave(sound)}
            onPlay={() => sound.sound_url && handlePlay(sound.sound_url, sound.id)}
            onFileUpload={(file) => handleFileUpload(sound, file)}
            onToggleActive={async (active) => {
              await updateSound.mutateAsync({ id: sound.id, is_active: active });
            }}
            isPlaying={playing === sound.id}
            isUploading={uploading === sound.id}
            isSaving={updateSound.isPending}
          />
        ))}
      </div>
...
function SoundRow({
  sound, urlInput, onUrlChange, onSave, onPlay, onFileUpload, onToggleActive,
  isPlaying, isUploading, isSaving,
}: {
  sound: any; urlInput: string; onUrlChange: (v: string) => void;
  onSave: () => void; onPlay: () => void; onFileUpload: (f: File) => void;
  onToggleActive: (active: boolean) => void;
  isPlaying: boolean; isUploading: boolean; isSaving: boolean;
}) {
    ...
    <div className="bg-slate-700 rounded-xl p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-3">
          ...
          {sound.sound_url && (
            <Button size="sm" variant="ghost" onClick={onPlay} className="text-amber-400">
      ...
      <div className="flex gap-2">
        <Input
          ...
          className="bg-slate-600 border-slate-500 flex-1"
