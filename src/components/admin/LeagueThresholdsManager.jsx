...
export function LeagueThresholdsManager() {
    ...
    const sortedThresholds = [...thresholds].sort((a, b) => leagueOrder.indexOf(a.league) - leagueOrder.indexOf(b.league));
    return (<div className="space-y-6">
      ...
      <div className="grid gap-4">
        {sortedThresholds.map((threshold) => {
            ...
            const hasChanges = edited && (edited.promotion !== threshold.promotion_xp_threshold ||
                edited.demotion !== threshold.demotion_xp_threshold);
            return (<div key={threshold.id} className="p-4 bg-slate-700 rounded-lg space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <Label className="text-slate-300">
                    ...
                    {threshold.league === "diamond" && (<span className="text-xs text-slate-500 ml-2">(N/A - top league)</span>)}
                  </Label>
                  <Input type="number" value={edited?.promotion ?? threshold.promotion_xp_threshold} onChange={(e) => setEditedThresholds({
                    ...editedThresholds,
                    [threshold.id]: {
                        ...edited,
                        promotion: parseInt(e.target.value) || 0,
                    },
                })} className="bg-slate-600 border-slate-500" disabled={threshold.league === "diamond"}/>
                ...
                <div>
                  <Label className="text-slate-300">
                    ...
                    {threshold.league === "bronze" && (<span className="text-xs text-slate-500 ml-2">(N/A - bottom league)</span>)}
                  </Label>
                  <Input type="number" value={edited?.demotion ?? threshold.demotion_xp_threshold} onChange={(e) => setEditedThresholds({
                    ...editedThresholds,
                    [threshold.id]: {
                        ...edited,
                        demotion: parseInt(e.target.value) || 0,
                    },
                })} className="bg-slate-600 border-slate-500" disabled={threshold.league === "bronze"}/>
                  <p className="text-xs text-slate-400 mt-1">
                    Below this = demotion
                  </p>
                </div>
              ...
              {hasChanges && (<Button onClick={() => handleSave(threshold.id)} disabled={updateThreshold.isPending} className="bg-amber-600 hover:bg-amber-700" size="sm">
                  {updateThreshold.isPending ? (<Loader2 className="w-4 h-4 animate-spin mr-2"/>) : (<Save className="w-4 h-4 mr-2"/>)}
                  Save Changes
                </Button>)}
            </div>);
        })}
