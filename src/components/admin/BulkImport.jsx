...
export function BulkImport() {
    ...
    const importLessons = async (data) => {
        let success = 0;
        const errors = [];
        // Process in batches
        const batches = [];
    ...
    const handleImport = async () => {
        ...
        try {
            ...
            const importResult = importType === "lessons" ? await importLessons(data) : await importQuestions(data);
        ...
        <div className="space-y-4">
          ...
          <Tabs defaultValue="json" className="w-full">
            ...
            <TabsContent value="json" className="space-y-3">
              <Textarea value={jsonContent} onChange={(e) => setJsonContent(e.target.value)} className="bg-slate-700 border-slate-600 min-h-[200px] font-mono text-sm" placeholder="Paste your JSON array here..."/>
            ...
            <TabsContent value="csv" className="space-y-3">
              <Textarea value={csvContent} onChange={(e) => setCsvContent(e.target.value)} className="bg-slate-700 border-slate-600 min-h-[200px] font-mono text-sm" placeholder="Paste your CSV content here..."/>
              <details className="p-3 bg-slate-700/50 rounded-lg">
                <summary className="text-xs text-slate-300 cursor-pointer select-none">
                  Show example format
                </summary>
            ...
            <TabsContent value="upload" className="space-y-3">
              <div onClick={() => fileInputRef.current?.click()} className="border-2 border-dashed border-slate-600 rounded-xl p-8 text-center cursor-pointer hover:border-amber-500 transition-colors">
          ...
          {/* Progress Bar */}
          {progress && isImporting && (<div className="space-y-2">
          ...
          {/* Import Button */}
          <Button onClick={handleImport} disabled={isImporting || (!jsonContent && !csvContent)} className="w-full bg-amber-600 hover:bg-amber-700">
            {isImporting ? (<>
          ...
          {result && (<div className="space-y-2">
              {result.success > 0 && (<div className="flex items-center gap-2 text-green-400">
                  <CheckCircle className="w-4 h-4"/>
                  <span>Successfully imported {result.success} items</span>
                </div>)}
              {result.errors.length > 0 && (<div className="space-y-1">
