...
export function HeartTimerPopover({ hearts, compact = false }) {
    const [isOpen, setIsOpen] = useState(false);
    const { data: profile } = useUserProfile();
    const { formattedTime, isRegenerating, timeUntilFull } = useHeartRegeneration(hearts, profile?.heart_regeneration_started_at);
    const maxHearts = 5;
    if (compact) {
        return (<div className="relative">
        <button onClick={() => setIsOpen(!isOpen)} className="flex items-center gap-1 text-destructive font-bold text-sm">
          <Heart className="w-5 h-5 fill-current"/>
          <span>{hearts}</span>
          {isRegenerating && (<span className="text-xs font-mono text-muted-foreground">
              ({formattedTime})
            </span>)}
        </button>
        ...
        {isOpen && (<>
            ...
            <div className="absolute top-full right-0 mt-2 z-50 w-64 p-4 bg-card rounded-2xl border border-border shadow-xl animate-scale-in">
              ...
              <div className="flex items-center justify-center gap-1 mb-4">
                {Array.from({ length: maxHearts }).map((_, i) => (<Heart key={i} className={cn("w-8 h-8 transition-all", i < hearts ? "text-destructive fill-destructive" : "text-muted-foreground")}/>))}
              </div>
              
              {/* Timer info */}
              {isRegenerating ? (<div className="space-y-3">
                ...
                </div>) : (<div className="text-center p-3 bg-primary/10 rounded-xl">
    ...
    return (<div className="flex items-center gap-1 text-destructive font-bold text-sm">
      <Heart className="w-5 h-5 fill-current"/>
      <span>{hearts}</span>
    </div>);
}
