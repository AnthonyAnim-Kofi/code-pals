...
function StudyHeatmap() {
    ...
    if (isLoading || !data) {
        return (<div className="flex items-center justify-center h-[120px] rounded-xl bg-muted/30">
    ...
    return (<div className="space-y-2">
      ...
      <div className="flex flex-wrap gap-1" style={{ width: "100%", maxWidth: HEATMAP_WEEKS * 14 }}>
        {cells.map((cell) => {
            const level = cell.minutes <= 0
                ? 0
                : cell.minutes >= maxMinutes
                    ? 4
                    : Math.ceil((cell.minutes / maxMinutes) * 4);
            return (<div key={cell.date} title={`${cell.date}: ${cell.minutes} min`} className={cn("w-3 h-3 rounded-sm border border-border/50 transition-colors", level === 0 && "bg-muted/40", level === 1 && "bg-primary/30", level === 2 && "bg-primary/50", level === 3 && "bg-primary/70", level === 4 && "bg-primary")}/>);
        })}
      </div>
    </div>);
}
function TimePerLanguageChart() {
    const { data, isLoading } = useStudyStats();
    if (isLoading || !data) {
        return (<div className="flex items-center justify-center h-[200px] rounded-xl bg-muted/30">
        <Loader2 className="w-6 h-6 animate-spin text-muted-foreground"/>
      </div>);
    }
    ...
    return (<ChartContainer config={chartConfig} className="h-[200px] w-full">
      <BarChart data={byLanguage} margin={{ top: 8, right: 8, bottom: 24, left: 8 }}>
        <XAxis dataKey="languageName" tickLine={false} axisLine={false} tick={{ fontSize: 12 }}/>
