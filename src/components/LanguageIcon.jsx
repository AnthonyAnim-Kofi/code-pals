...
export function LanguageIcon({ slug, icon, className = "", size = 32 }) {
    const normalizedSlug = slug.toLowerCase().trim();
    const deviconId = LANGUAGE_ICON_MAP[normalizedSlug];
    if (deviconId) {
        return (<img src={`https://cdn.jsdelivr.net/gh/devicons/devicon/icons/${deviconId}/${deviconId}-original.svg`} alt={slug} width={size} height={size} className={className} onError={(e) => {
