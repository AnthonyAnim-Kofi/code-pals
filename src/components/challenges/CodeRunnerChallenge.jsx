...
import { Loader2, CheckCircle, XCircle, RotateCcw, Lightbulb } from "lucide-react";
...
export function CodeRunnerChallenge({ initialCode, expectedOutput, hint, instruction, language = "python", onAnswer, disabled = false, }) {
    ...
    return (<div className="flex flex-col gap-0 -mx-4 sm:mx-0">
      {/* Tabs */}
      <div className="flex items-center gap-1 px-4 sm:px-0 mb-4">
        <button onClick={() => setActiveTab("learn")} className={cn("px-5 py-2 rounded-full text-sm font-bold transition-all", activeTab === "learn"
            ? "bg-primary text-primary-foreground shadow-sm"
            : "text-muted-foreground hover:bg-muted")}>
          📖 Learn
        </button>
        <button onClick={() => setActiveTab("code")} className={cn("px-5 py-2 rounded-full text-sm font-bold transition-all", activeTab === "code"
            ? "bg-primary text-primary-foreground shadow-sm"
            : "text-muted-foreground hover:bg-muted")}>
          &lt;/&gt; Code
        </button>
        {(language === "html" || language === "css") && (<button onClick={() => setActiveTab("preview")} className={cn("px-5 py-2 rounded-full text-sm font-bold transition-all", activeTab === "preview"
                ? "bg-primary text-primary-foreground shadow-sm"
                : "text-muted-foreground hover:bg-muted")}>
            👁 Preview
          </button>)}
      ...
      {/* Learn Tab */}
      {activeTab === "learn" && (<div className="px-4 sm:px-0 space-y-4 animate-fade-in">
      ...
      {/* Code Tab */}
      {activeTab === "code" && (<div className="space-y-0 animate-fade-in">
          {/* Editor header */}
          ...
          <div className="mx-4 sm:mx-0">
            <CodeEditor value={code} onChange={(val) => !disabled && !hasChecked && setCode(val)} language={language} readOnly={disabled || hasChecked} minHeight="180px" className="rounded-t-none rounded-b-xl"/>
          </div>

          {/* Quick-insert shortcuts */}
          <div className="px-4 sm:px-0 pt-3">
            <div className="flex flex-wrap gap-1.5">
              {shortcuts.map((shortcut) => (<button key={shortcut} onClick={() => insertShortcut(shortcut)} disabled={disabled || hasChecked} className={cn("px-3 py-1.5 rounded-lg text-xs font-bold border-2 transition-all", "bg-card border-border text-foreground hover:border-primary/50 hover:bg-primary/5", "active:scale-95 disabled:opacity-40 disabled:cursor-not-allowed", shortcut.length > 2 && "bg-primary/10 border-primary/30 text-primary")}>
                  {shortcut}
                </button>))}
            </div>
          </div>
          ...
          {/* Output */}
          {output !== null && (<div className={cn("mx-4 sm:mx-0 mt-3 rounded-xl p-4 border-2", hasChecked && isCorrect && "bg-primary/10 border-primary/30", hasChecked && !isCorrect && "bg-destructive/10 border-destructive/30", !hasChecked && "bg-muted border-border")}>
              ...
              {hasChecked && !isCorrect && (<p className="text-xs text-muted-foreground mt-2">
          ...
          <div className="px-4 sm:px-0 pt-4 pb-2">
            <div className="flex gap-2">
              {hint && (<Button variant="outline" size="lg" className="rounded-xl" onClick={() => setShowHint(!showHint)}>
                  <Lightbulb className="w-5 h-5 text-primary"/>
                </Button>)}
              <Button onClick={runCode} disabled={disabled || isRunning || hasChecked} className={cn("flex-1 rounded-xl font-extrabold text-base h-12", hasChecked && isCorrect && "bg-primary", hasChecked && !isCorrect && "bg-destructive")} variant={hasChecked ? (isCorrect ? "default" : "destructive") : "default"} size="lg">
                {isRunning ? (<>
            ...
            {/* Hint display */}
            {showHint && hint && (<div className="mt-3 p-3 bg-primary/10 border-2 border-primary/20 rounded-xl animate-fade-in">
      ...
      {/* Preview Tab */}
      {activeTab === "preview" && (<div className="px-4 sm:px-0 animate-fade-in">
          {htmlPreview ? (<div className="rounded-2xl border-2 border-border overflow-hidden">
            ...
            </div>) : (<div className="rounded-2xl border-2 border-dashed border-border p-12 text-center">
