...
function QuestCard({ quest, progress, onClaim, claimingId }) {
    ...
    return (<div className={cn("p-4 bg-card rounded-2xl border border-border card-elevated transition-all", isCompleted && !isClaimed && "bg-primary/5 border-primary/30", isClaimed && "opacity-60")}>
      <div className="flex items-start gap-4">
        <div className={cn("w-12 h-12 rounded-xl flex items-center justify-center shrink-0", isCompleted ? "bg-primary" : "bg-muted")}>
          {isCompleted ? (<CheckCircle2 className="w-6 h-6 text-primary-foreground"/>) : (<Target className="w-6 h-6 text-muted-foreground"/>)}
        </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="space-y-2">
            <Progress value={progressPercent} size="sm" indicatorColor={isClaimed ? "secondary" : isCompleted ? "primary" : "gradient"}/>
