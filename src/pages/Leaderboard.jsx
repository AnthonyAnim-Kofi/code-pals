...
import { Trophy, Medal, Loader2, Users, ArrowUp, ArrowDown, Clock, Crown, Gift, Star, Gem, Heart, Flame } from "lucide-react";
...
import { useLeagueChestConfig } from "@/hooks/useLeagueChests";
...
function UserAvatar({ user, size = "md" }) {
    ...
    return (<div className={cn("rounded-full bg-muted flex items-center justify-center overflow-hidden", sizeClasses[size])}>
      {user.avatar_url ? (<img src={user.avatar_url} alt="" className="w-full h-full object-cover"/>) : (<img src={mascot} alt="" className={cn("object-contain", imgSize[size])}/>)}
    </div>);
}
/* ── Podium card for top 3 ── */
function PodiumCard({ user, rank, isCurrentUser, xp, }) {
    ...
    return (<div className={cn("flex flex-col items-center relative", isFirst ? "order-2 -mt-4 z-10" : rank === 2 ? "order-1 mt-6" : "order-3 mt-6", isFirst ? "pb-8" : "pb-4")}>
      {isFirst && (<div className="mb-1 animate-bounce">
      ...
      <div className="relative">
        ...
        <div className={cn("absolute -bottom-2.5 left-1/2 -translate-x-1/2 w-7 h-7 rounded-full flex items-center justify-center text-xs font-black shadow-md", style.badge)}>
          {rank}
        </div>
      </div>

      <p className={cn("mt-4 text-sm font-bold truncate max-w-[100px] text-center", isCurrentUser ? "text-primary" : "text-foreground")}>
        {user.display_name || user.username || "Learner"}
      </p>
      {isCurrentUser && (<span className="text-[10px] font-semibold text-primary uppercase tracking-wider">You</span>)}

      <div className={cn("mt-1.5 px-3 py-1 rounded-full text-xs font-black tracking-wide", isFirst
...
export default function Leaderboard() {
    ...
    const getXp = (u) => (isWeekly ? u.weekly_xp : u.xp) ?? 0;
    if (isLoading) {
    ...
    return (<div className="space-y-5 pb-8 max-w-2xl mx-auto">
      {/* Header */}
      ...
      <div className="flex justify-center">
        <div className="inline-flex items-center rounded-full bg-muted p-1 text-sm">
          <button type="button" onClick={() => setTimeRange("weekly")} className={cn("px-4 py-1.5 rounded-full font-semibold transition-all", isWeekly ? "bg-background text-foreground shadow-sm" : "text-muted-foreground")}>
            This Week
          </button>
          <button type="button" onClick={() => setTimeRange("allTime")} className={cn("px-4 py-1.5 rounded-full font-semibold transition-all", !isWeekly ? "bg-background text-foreground shadow-sm" : "text-muted-foreground")}>
            All Time
          </button>
        </div>
      </div>
      ...
      {/* Week countdown */}
      {isWeekly && (<div className="flex items-center gap-2 px-4 py-2.5 bg-muted/60 rounded-xl text-sm">
      ...
      <div className="bg-card rounded-2xl border border-border p-4">
        <div className="flex items-center justify-between">
          ...
          {currentUserRank > 0 && (<div className="text-right">
              <p className="text-2xl font-black text-foreground">#{currentUserRank}</p>
              <div className="flex flex-col items-end mt-1 gap-1">
                {isWeekly && selectedLeague !== "diamond" && (<span className={cn("text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full flex items-center gap-1", userWeeklyXp >= promotionXp
                    ? "bg-green-500/10 text-green-500"
                    : "bg-muted text-muted-foreground")}>
                    <ArrowUp className="w-3 h-3"/> {userWeeklyXp >= promotionXp ? "Promoting" : `Promotion: ${promotionXp} XP`}
                  </span>)}
                {isWeekly && selectedLeague !== "bronze" && (<span className={cn("text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full flex items-center gap-1", userWeeklyXp < demotionXp
      ...
      {isWeekly && (<div className="w-full">
          <Tabs value={selectedLeague} onValueChange={setSelectedLeague}>
            <TabsList className="w-full">
              {LEAGUES.map((league) => (<TabsTrigger key={league.key} value={league.key} className="flex-1 capitalize">
                  {league.name}
                </TabsTrigger>))}
            </TabsList>
          </Tabs>
      ...
      {/* Content */}
      {users.length === 0 ? (<div className="p-10 bg-card rounded-2xl border border-border text-center">
          ...
          {/* ── Top 3 Podium ── */}
          {topThree.length >= 3 && (<div className="bg-card rounded-2xl border border-border p-6 pt-8">
              <div className="flex justify-center items-end gap-3 sm:gap-6">
                {topThree.map((u, i) => (<PodiumCard key={u.id} user={u} rank={i + 1} isCurrentUser={u.user_id === profile?.user_id} xp={getXp(u)}/>))}
              </div>
            </div>)}

          {/* ── Weekly Chest Rewards Preview ── */}
          {isWeekly && chestConfigs && chestConfigs.length > 0 && (
            <div className="bg-card rounded-2xl border border-border overflow-hidden">
              ...
              <div className="p-4 space-y-3">
                {chestConfigs.map((chest) => (
                  <div key={chest.id} className="flex items-center gap-3 p-3 bg-muted/40 rounded-xl">
                    <div className={cn(
                      "w-10 h-10 rounded-full flex items-center justify-center text-lg font-black shrink-0",
                      chest.rank_position === 1 ? "bg-golden/15 text-golden" :
                      chest.rank_position === 2 ? "bg-muted-foreground/15 text-muted-foreground" :
                      "bg-amber-700/15 text-amber-700"
                    )}>
                      {chest.rank_position === 1 ? "🥇" : chest.rank_position === 2 ? "🥈" : "🥉"}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-bold text-foreground truncate">{chest.chest_name}</p>
                      <div className="flex items-center gap-3 mt-1 text-xs text-muted-foreground">
                        {chest.xp_reward > 0 && <span className="flex items-center gap-0.5"><Star className="w-3 h-3 text-golden" />{chest.xp_reward} XP</span>}
                        {chest.gems_reward > 0 && <span className="flex items-center gap-0.5"><Gem className="w-3 h-3 text-cyan-500" />{chest.gems_reward}</span>}
                        {chest.hearts_reward > 0 && <span className="flex items-center gap-0.5"><Heart className="w-3 h-3 text-destructive" />{chest.hearts_reward}</span>}
                        {chest.streak_freezes_reward > 0 && <span className="flex items-center gap-0.5"><Flame className="w-3 h-3 text-blue-500" />{chest.streak_freezes_reward}</span>}
                      </div>
                    </div>
                  </div>
                ))}
          ...
          <div className="bg-card rounded-2xl border border-border overflow-hidden">
            <div className="px-4 py-2.5 border-b border-border flex flex-wrap items-center justify-between gap-2">
              ...
              {isWeekly && (<div className="flex items-center gap-3 text-[10px] font-bold uppercase tracking-wider">
                  {selectedLeague !== "diamond" && (<span className="flex items-center gap-1 text-green-500/80">
                      <ArrowUp className="w-3 h-3"/> Advance: {promotionXp} XP
                    </span>)}
                  {selectedLeague !== "bronze" && (<span className="flex items-center gap-1 text-destructive/80">
            ...
            {users.map((leaderUser, index) => {
                ...
                return (<div key={leaderUser.id} className={cn("flex items-center gap-3 px-4 py-3 border-b border-border last:border-b-0 transition-colors", isCurrentUser && "bg-primary/5", inPromotionZone && "border-l-2 border-l-green-500", inDemotionZone && "border-l-2 border-l-destructive")}>
