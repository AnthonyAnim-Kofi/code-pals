...
export default function Practice() {
    ...
    const practiceableLessons = useMemo(() => (dbLessons?.filter((lesson) => completedIds.has(lesson.id)) || []), [dbLessons, completedIds]);
    ...
    const lessonsByLanguage = useMemo(() => {
        ...
        return Array.from(map.values()).sort((a, b) => a.languageName.localeCompare(b.languageName));
    ...
    return (<div className="space-y-8">
      ...
      <div className="space-y-6">
        ...
        {lessonsByLanguage.map((group) => (<div key={group.languageId} className="space-y-3">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              {group.lessons.map((lesson) => {
                const progress = completedLessons.find((p) => String(p.lesson_id) === lesson.id);
                return (<div key={lesson.id} className="p-4 rounded-2xl border transition-all bg-card border-border hover:border-primary/50 cursor-pointer">
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {progress && (<p className="text-xs text-muted-foreground mt-1">
                            Best: {progress.accuracy}% accuracy •{" "}
                            {progress.xp_earned} XP
                          </p>)}
                      </div>
                      <Button size="sm" onClick={() => handlePractice(lesson.id)} className="rounded-lg shrink-0">
      ...
      {/* Empty State */}
      {practiceableLessons.length === 0 && (<div className="p-8 bg-card rounded-2xl border border-border text-center">
