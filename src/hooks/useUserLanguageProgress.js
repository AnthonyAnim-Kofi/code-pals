...
export function useUserLanguageProgress() {
    ...
    return useQuery({
        ...
        `)
            ...
            if (progressError)
                throw progressError;
            const completedLessonIds = new Set(lessonProgress?.map((p) => p.lesson_id?.toString()) || []);
                ...
                (lang.units || []).forEach((unit) => {
                    (unit.lessons || []).forEach((lesson) => {
                        ...
                        if (completedLessonIds.has(lesson.id)) {
                            completedCount++;
                            const progress = lessonProgress?.find((p) => p.lesson_id?.toString() === lesson.id);
...
export function useActiveLanguage() {
    ...
    return useQuery({
        ...
        queryFn: async () => {
            ...
            if (profileError)
                throw profileError;
            const activeLanguageId = profile?.active_language_id;
            if (activeLanguageId) {
